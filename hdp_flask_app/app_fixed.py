...
def search_knowledge(query):
    ...
    # 3. جستجوی کلمات کلیدی (Keyword Match)
    keywords = q.split()
    for key, value in knowledge.items():
        for kw in keywords:
...
def chat():
    ...
    <body>
        <div class="chat-container">
            <div class="header"><h3>🚕 HDP Chat</h3><p>گویش بندری | هوش مصنوعی</p></div>
            <div class="messages" id="messages">
                <div class="message bot"><div class="bubble">👋 سَلام خَری! به HDP ONE خوش اومدی. چِطوری مِردُم؟</div></div>
            </div>
...
def chat_api():
    ...
    # 2. سرویس‌های سریع
    if 'هوا' in msg:
        return jsonify({'response': '🌤️ بندرعباس: 32°C، آفتابی و شرجی', 'source': 'weather'})
    if 'طلا' in msg:
        return jsonify({'response': '💰 طلا 18 عیار: 4,850,000 تومان\n💵 دلار: 78,000 تومان', 'source': 'gold'})
    if 'ترافیک' in msg:
        return jsonify({'response': '🚦 چهارراه غزی سنگین، بلوار ساحلی روان', 'source': 'traffic'})
    ...
    # 4. پاسخ پیش‌فرض
    return jsonify({'response': '🤔 متوجه نشدم داداش!\n\n📌 می‌تونی بپرسی:\n• سلام\n• ترافیک\n• آب و هوا\n• قیمت طلا\n• جزیره قشم\n• بندرعباس', 'source': 'fallback'})
